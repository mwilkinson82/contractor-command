    ...
    <Container className="py-10">
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-border pb-6">
        ...
        <div className="flex gap-2">
          <Link to="/templates" className="rounded-md border border-border bg-card px-3 py-1.5 text-[12px] hover:bg-muted">View Templates</Link>
          <Link to="/replays" className="rounded-md border border-border bg-card px-3 py-1.5 text-[12px] hover:bg-muted">View Replays</Link>
        </div>
      </div>
      ...
      <div className="mt-6">
        {tab === "templates" ? <TemplatesAdmin /> : <ReplaysAdmin />}
      </div>
    </Container>
  );
}
...
function TemplateRowEditor({ row, onChange }: { row: TemplateRow; onChange: () => void }) {
  ...
  async function uploadFile(file: File) {
    ...
    await supabase.from("templates").update({ download_url: path, file_type: ext }).eq("id", row.id);
    setUploading(false);
    onChange();
  }
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-4">
        <label className={`inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-border bg-background px-3 py-1.5 text-xs hover:bg-muted ${uploading ? "opacity-60" : ""}`}>
        ...
        {hasFile && (
          <span className="inline-flex items-center gap-1 rounded-full bg-foreground/5 px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
            {isExternal ? <><ExternalLink className="h-3 w-3" /> external</> : "stored"}
          </span>
        )}
        <button
...
function ReplayRowEditor({ row, onChange }: { row: ReplayRow; onChange: () => void }) {
  ...
  const dirty =
    JSON.stringify({ ...draft, tags }) !== JSON.stringify({ ...row, tags: row.tags });
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <Field label="Recorded at">
          <input
            ...
            onChange={(e) => setDraft({ ...draft, recorded_at: new Date(e.target.value).toISOString() })}
            className="w-full rounded border border-border bg-background px-2 py-1.5 text-sm focus:border-ink focus:outline-none"
          />
        </Field>
