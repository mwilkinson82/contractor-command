...
BEGIN
  ...
  outcomes AS (
    SELECT
      *,
      ...
      (
        visible_now
        AND (ttl_expired OR missing_required_fields OR failed_attempts >= 5)
      ) AS would_move_to_dlq,
      (
        visible_now
        AND already_sent
        AND NOT ttl_expired
      ...
      (
        ...
        AND NOT missing_required_fields
        AND failed_attempts < 5
        AND (suppressed_recipient OR used_unsubscribe_token)
      ) AS would_suppress_now,
      (
        visible_now
        AND NOT already_sent
        AND NOT ttl_expired
        AND NOT missing_required_fields
        AND failed_attempts < 5
        AND NOT (suppressed_recipient OR used_unsubscribe_token)
      ) AS would_send
    FROM classified
  ),
  totals AS (
  ...
  SELECT jsonb_build_object(
    ...
    'totals', (
      SELECT jsonb_build_object(
        'total', total,
        'visibleNow', visible_now,
        'hiddenUntilVisible', hidden_until_visible,
        'wouldSend', would_send,
        'wouldMoveToDlq', would_move_to_dlq,
        'wouldSkipDuplicate', would_skip_duplicate,
        'wouldSuppress', would_suppress,
        'missingQueuedAt', missing_queued_at,
    ...
    'templates', COALESCE((
      ...
      FROM (
        SELECT
          ...
          COUNT(*) FILTER (WHERE would_suppress_now)::INT AS would_suppress
        FROM outcomes
        GROUP BY queue_name, template_name
      ) template_summary
    ), '[]'::JSONB),
    'samples', COALESCE((
      SELECT jsonb_agg(
